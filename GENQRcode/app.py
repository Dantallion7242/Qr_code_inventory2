...
def index():
    if 'user_id' in session:
        return render_template('index.html')  # Redirect to dashboard or another page
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('register'))  # Redirect back to register if the username exists
        finally:
            conn.close()
    ...
    if request.method == 'POST':
        ...
        if entered_key == unique_key:
            ...
            if item:
                ...
                flash('Item not found. Please try again.', 'danger')
        else:
